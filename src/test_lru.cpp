...
template <typename ValuesType, typename LRUType>
size_t TestLRU(const ValuesType& values, LRUType&& lru) {
	size_t hit = 0;
	size_t sum = 0;
...
int main() {
	for (auto sigma : sigmas) {
		...
		TestLRU(v1, LRUMapVector<std::string, size_t>(LRU_CAPACITY));
		std::cout << "LRUMapList\t";
		TestLRU(v1, LRUMapList<std::string, size_t>(LRU_CAPACITY));
		std::cout << "LRUHashVector\t";
		TestLRU(v1, LRUHashVector<std::string, size_t>(LRU_CAPACITY));
		std::cout << "LRUHashList\t";
		TestLRU(v1, LRUHashList<std::string, size_t>(LRU_CAPACITY));
		std::cout << std::endl;
		std::cout << "LRUExMapVector\t";
		TestLRU(v1, LRUExMapVector<std::string, size_t>(LRU_CAPACITY - 100, 100));
		std::cout << std::endl;
	}
	return 0;
}
